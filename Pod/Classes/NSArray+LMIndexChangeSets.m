...
+ (NSDictionary *)
indexChangeSetsForUpdatedList:(NSArray *)updated
                 previousList:(NSArray *)previous
           identityComparator:(NSComparisonResult (^)(id a, id b))comparator {
  NSIndexSet *deleted = [previous
      indexesOfObjectsPassingTest:^BOOL(id prev, NSUInteger idx, BOOL *stop) {
        if (updated == nil ||
            [updated indexOfObjectPassingTest:^BOOL(id curr, NSUInteger idx,
                                                    BOOL *stop) {
  ...
  NSIndexSet *inserted = [updated
      indexesOfObjectsPassingTest:^BOOL(id prev, NSUInteger idx, BOOL *stop) {
        if (previous == nil ||
            [previous indexOfObjectPassingTest:^BOOL(id curr, NSUInteger idx,
                                                     BOOL *stop) {
              return (comparator(curr, prev) == NSOrderedSame);
            }] == NSNotFound) {
          return YES;
